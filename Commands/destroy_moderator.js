    ...
    if (room !== null) {
        ...
        for (let i = 0; i < roomItems.length; i++) {
            ...
            if (parsedInput.endsWith(roomItems[i].identifier) && roomItems[i].identifier !== "") {
                if (roomItems[i].inventory.length === 0 || roomItems[i].prefab.preposition === "") return message.reply(`${roomItems[i].identifier ? roomItems[i].identifier : roomItems[i].prefab.id} cannot hold items.`);
                ...
                if (parsedInput.endsWith(" OF")) {
                    ...
                    if (containerItemSlot === null) return message.reply(`couldn't find "${newArgs[newArgs.length - 1]}" of ${containerItem.identifier ? containerItem.identifier : containerItem.prefab.id}.`);
                }
        ...
        if (destroyAll) {
            if (parsedInput !== "") return message.reply(`couldn't find "${parsedInput}" at ${room.name}`);
            for (let i = 0; i < containerItems.length; i++)
                itemManager.destroyItem(containerItems[i], true);
            message.channel.send(`Successfully destroyed ${containerItems.length} items ${preposition} ${containerName}.`);
        }
        else {
            ...
            if (item === null) return message.reply(`couldn't find item "${parsedInput}" ${preposition} ${containerName}.`);

            itemManager.destroyItem(item, true);
            message.channel.send(`Successfully destroyed ${item.identifier ? item.identifier : item.prefab.id} ${preposition} ${containerName}.`);
        }
    }
    else {
        ...
        for (let i = 0; i < playerItems.length; i++) {
            ...
            if (parsedInput.endsWith(playerItems[i].identifier) && playerItems[i].identifier !== "" || parsedInput.endsWith(playerItems[i].prefab.id)) {
                if (playerItems[i].inventory.length === 0 || playerItems[i].prefab.preposition === "") return message.reply(`${playerItems[i].identifier ? playerItems[i].identifier : playerItems[i].prefab.id} cannot hold items.`);
        ...
        if (containerItem !== null) {
            ...
            for (let i = 0; i < player.inventory.length; i++) {
                if (player.inventory[i].name === parsedInput) {
                    ...
                    if (item === null) return message.reply(`cannot destroy item equipped to ${equipmentSlotName} because nothing is equipped to it.`);
                    if (destroyAll) return message.reply(`the "all" argument cannot be used when the container is an equipment slot.`);
                    break;
                }
                    ...
                    item = player.inventory[i].equippedItem;
                    equipmentSlotName = player.inventory[i].name;
                    if (destroyAll) return message.reply(`the "all" argument cannot be used when the container is an equipped item.`);
                    break;
                }
            }
            if (item !== null && equipmentSlotName !== "") {
                itemManager.destroyInventoryItem(item, bot, true);
                message.channel.send(`Successfully destroyed ${item.identifier ? item.identifier : item.prefab.id} equipped to ${player.name}'s ${equipmentSlotName}.`);
                return;
            }
        }
        ...
        else return message.reply(`couldn't find "${parsedInput}" in ${player.name}'s inventory.`);
    }
