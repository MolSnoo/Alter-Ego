...
export async function execute (game, message, command, args, player) {
    ...
    if (item === null) return addReply(game, message, `Couldn't find item "${itemName}" in either of your hands. If this item is elsewhere in your inventory, please unequip or unstash it before trying to equip it.`);
    ...
    for (let i = 0; i < player.inventory.length; i++) {
        if (slotName && player.inventory[i].id === slotName) {
            ...
            if (!acceptableSlot) return addReply(game, message, `${itemName} can't be equipped to equipment slot ${slotName}.`);
            if (player.inventory[i].equippedItem !== null) return addReply(game, message, `Cannot equip items to ${slotName} because ${player.inventory[i].equippedItem.name} is already equipped to it.`);
        }
    }
    if (!foundSlot) return addReply(game, message, `Couldn't find equipment slot "${slotName}".`);

    const action = new Action(game, ActionType.Equip, message, player, player.location, false);
