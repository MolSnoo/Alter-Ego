...
module.exports.run = async (bot, game, message, command, args) => {
    ...
    game.logChannel.send(`${time} - ${player.name} forcefully crafted ${productPhrase} from ${item1Name} and ${item2Name} in ${player.location.channel}`);

    message.channel.send(`Successfully crafted ${productPhrase} from ${item1Name} and ${item2Name} for ${player.name}.`);
