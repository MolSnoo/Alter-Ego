...
export async function execute (game, command, args, player, callee) {
    ...
    if (args[1] === "x" && args[2]) {
        ...
        if (isNaN(x)) return messageHandler.addGameMechanicMessage(game.guildContext.commandChannel, `Error: Couldn't execute command "${cmdString}". "${args[2]}" is not a valid X-coordinate.`);
        ...
        if (isNaN(y)) return messageHandler.addGameMechanicMessage(game.guildContext.commandChannel, `Error: Couldn't execute command "${cmdString}". "${args[2]}" is not a valid Y-coordinate.`);
        ...
        if (isNaN(z)) return messageHandler.addGameMechanicMessage(game.guildContext.commandChannel, `Error: Couldn't execute command "${cmdString}". "${args[2]}" is not a valid Z-coordinate.`);

        for (let i = 0; i < players.length; i++)
            players[i].pos.z = z;
    }
    else if ((args[1] === "x" || args[1] === "y" || args[1] === "z") && !args[2])
        return messageHandler.addGameMechanicMessage(game.guildContext.commandChannel, `Error: Couldn't execute command "${cmdString}". An individual coordinate was specified, but no number was given.`);
    else {
        let coordinates = args.slice(1);
        if (coordinates.length !== 3) return messageHandler.addGameMechanicMessage(game.guildContext.commandChannel, `Error: Couldn't execute command "${cmdString}". Invalid coordinates given.`);
        for (let i = 0; i < coordinates.length; i++) {
            if (isNaN(coordinates[i])) return messageHandler.addGameMechanicMessage(game.guildContext.commandChannel, `Error: Couldn't execute command "${cmdString}". Invalid coordinates given.`);
        }
