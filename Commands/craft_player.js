...
module.exports.run = async (bot, game, message, command, args, player) => {
    ...
    if (item1 === null && item2 !== null) {
        ...
        return message.reply(`couldn't find item "${item1Name}" in either of your hands.`);
    }
    else if (item1 !== null && item2 === null) {
        item2Name = parsedInput.replace(item1.name, "").replace(" WITH ", "").replace(" AND ", "");
        return message.reply(`couldn't find item "${item2Name}" in either of your hands.`);
    }
    else if (item1 === null && item2 === null) {
        ...
        item1Name = args[0];
        item2Name = args[1];
        return message.reply(`couldn't find items "${item1Name}" and "${item2Name}" in either of your hands.`);
    }
