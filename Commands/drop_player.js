...
module.exports.run = async (bot, game, message, command, args, player) => {
    ...
    else return message.reply(`couldn't find item "${parsedInput}" in either of your hands. If this item is elsewhere in your inventory, please unequip or unstash it before trying to drop it.`);
    ...
    if (parsedInput !== "") {
        for (let i = 0; i < objects.length; i++) {
            ...
            else if (parsedInput === `${newArgs[0]} ${objects[i].name}` && objects[i].preposition === "") return message.reply(`${objects[i].name} cannot hold items. Contact a moderator if you believe this is a mistake.`);
        }
    }
    ...
    if (parsedInput !== "") {
        for (let i = 0; i < items.length; i++) {
            if (items[i].name === parsedInput) return message.reply(`you need to supply a preposition.`);
