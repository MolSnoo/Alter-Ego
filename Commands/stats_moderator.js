...
export async function execute(game, message, command, args) {
    ...
    statsString += `Per: ${player.defaultPerception}\n`;
    ...
    const perModifier = player.getStatModifier(player.perception);
    ...
    statsString += `Per: ${player.perception} [` + (perModifier > 0 ? '+' : '') + `${perModifier}]\n`;
    ...
    game.communicationHandler.sendToCommandChannel(statsString);
}
