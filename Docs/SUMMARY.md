# Summary

[Introduction](README.md)

# About

- [Neo World Program](about/nwp.md)
- [Discord](about/discord.md)

# Player Guide

- [Playing the Game]()

# Moderator Guide

- [Installation and Setup](moderator_guide/installation.md)
- [Moderating](moderator_guide/moderating.md)
- [Writing Descriptions](moderator_guide/writing_descriptions.md)
- [Edit Mode](moderator_guide/edit_mode.md)
- [Mapmaking]()

# Developer Reference

- [Commands](developer_reference/commands/index.md)
    - [Bot Commands](developer_reference/commands/bot_commands.md)
    - [Eligible Commands](developer_reference/commands/eligible_commands.md)
    - [Moderator Commands](developer_reference/commands/moderator_commands.md)
    - [Player Commands](developer_reference/commands/player_commands.md)
- [Data Structures](developer_reference/data_structures/index.md)
    - [Die](developer_reference/data_structures/die.md)
    - [Equipment Slot](developer_reference/data_structures/equipment_slot.md)
    - [Event](developer_reference/data_structures/event.md)
    - [Exit](developer_reference/data_structures/exit.md)
    - [Gesture](developer_reference/data_structures/gesture.md)
    - [Inventory Item](developer_reference/data_structures/inventory_item.md)
    - [Item](developer_reference/data_structures/item.md)
    - [Narration]()
    - [Object](developer_reference/data_structures/object.md)
    - [Player](developer_reference/data_structures/player.md)
    - [Prefab](developer_reference/data_structures/prefab.md)
    - [Puzzle](developer_reference/data_structures/puzzle.md)
    - [Recipe](developer_reference/data_structures/recipe.md)
    - [Room](developer_reference/data_structures/room.md)
    - [Status](developer_reference/data_structures/status.md)
    - [Whisper](developer_reference/data_structures/whisper.md)
- [Settings]()
    - [Docker Settings](developer_reference/settings/docker_settings.md)
    - [Node Settings](developer_reference/settings/node_settings.md)

# Appendix

- [Manual Installation](appendix/manual_installation/index.md)
    - [Node Installation](appendix/manual_installation/node.md)
    - [Manual Channel and Role Creation](appendix/manual_installation/channel_and_role_creation.md)