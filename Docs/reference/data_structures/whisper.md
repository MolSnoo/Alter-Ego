...
A Whisper can also be created when a Player [hides]() in an [Object](object.md). This allows a Whisper to be
...
the [Whisper category](../settings/docker_settings.md#whisper_category). In this channel, Players can speak to each other
...
permission would allow other Players in the Whisper to see their [Discord](../../about/discord.md) account, thus revealing their identity.
...
the [autoDeleteWhisperChannels setting](../settings/docker_settings.md#autodelete_whisper_channels) is `true`, then the
