...
more [[Players|Data-Structure:-Player]] speaking quietly to each other such that no one else in
the [[Room|Data-Structure:-Room]] can hear them.

A normal Whisper can only be created when a Player or [[moderator|Tutorial:-Moderating]] uses
the [[whisper|Commands#whisper-player-command]] [[command|Commands#whisper-moderator-command]]. There is no upper limit
to the number of Players that can be included in a Whisper, so long as they are all in the same Room. However, it is not
possible for one Player to create a Whisper with Players in the Room who have the `hidden`, `concealed`, `no hearing`,
or `unconscious` [[behavior attributes|Data-Structure:-Status#behavior-attributes]]. If a Player in a Whisper becomes
inflicted with a [[Status Effect|Data-Structure:-Status]] with one of these behavior attributes or they leave the Room,
they will be removed from the Whisper. If, when a Player is removed from the Whisper, the group of Players remaining is
the same as a different Whisper that already exists, it will be deleted upon the Player's removal. Otherwise, a Whisper
will only be deleted once all Players have been removed from it.

A Whisper can also be created when a Player [[hides|hiding]] in an [[Object|Data-Structure:-Object]]. This allows a
Whisper to be created with only one Player. However, if more Players hide in the same Object, the Whisper will be
...
This is the name that the channel will be set to. It is created using the [[name|Data-Structure:-Room#name]] of the
Whisper's location followed by an alphabetized list of all the
Players' [[display names|Data-Structure:-Player#display-name]], all separated with hyphens (`-`). Whenever a Player is
removed from the Whisper, the channel name is updated to exclude them.
