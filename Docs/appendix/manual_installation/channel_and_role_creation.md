...
This article details the process of manually setting up a [[Discord]] server for the [[Neo World Program]]. If you use
the server template provided in
the [[official tutorial|Tutorial:-Installation-and-setup#step-4-create-a-discord-server]], you can skip this process
entirely.
...
* Use External Emojis _(optional)_
    * If this permission is not enabled, then any external emojis that are sent in a Room channel, whether by a Player
      or a [[moderator|Tutorial:-Moderating]], will not be sent
      in [[spectate channels|Data-Structure:-Player#spectate-channel]]. Instead, they will be replaced with the name of
      the emoji.
...
This is the role for testers. This role is only necessary if you use debug mode. For more information, see the article
on [[debug mode|Debug mode]]. Enable these settings:
...
This role allows a player to move to any room they wish, adjacent or not. This should generally only be given to the
host(s) of your killing game, be it Monokuma, Monomi, or anyone else. Enable these settings:
...
This is the last role you need to make. This should be given to your [[moderator(s)|Tutorial:-Moderating]], including
yourself. Enable these settings:
...
kinds of channels here such as rules for the killing game, a list of players and their talents,  [[maps|Map]], etc.
...
channels corresponding with the game's [[Rooms|Data-Structure:-Room]]. The reason you can create multiple categories for
...
This category is where Alter Ego will create [[Whisper|Data-Structure:-Whisper]] channels. There is only one permission
