...
by a moderator with the [load command](../reference/commands/moderator_commands.md#load). By keeping an internal copy of
...
ways (the other being the [save command](../reference/commands/moderator_commands.md#save)) of forcibly saving the game.
After the spreadsheet is updated, Alter Ego will pause its autosave functionality until edit mode is disabled. This
allows a moderator to manually edit the spreadsheet without worrying about their work being overwritten by the next
autosave.

In addition, [Players](../reference/data_structures/player.md) are unable to use commands during edit mode. They are
still able to speak (and thus use the [say command](../reference/commands/player_commands.md#say)), since that almost
...
[`unconscious` behavior attribute](../reference/data_structures/status.md#unconscious). As Players are normally able to
